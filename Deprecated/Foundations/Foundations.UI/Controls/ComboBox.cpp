...
ComboBox::ComboBox(int X, int Y, int Width, int Height, HWND Parent, HMENU ID, HINSTANCE ins, const std::vector<String>& Items, StyleSheet Style, TextStyleSheet TextStyle)
{
	if (!_ThisAtom)
		InitBase(ins);
...
LRESULT __stdcall ComboBox::ThisProc(HWND Window, UINT Message, WPARAM wp, LPARAM lp)
{
	...
	{
		const ::Char* Arr = reinterpret_cast<const ::Char*>(wp);
		String This(Arr);
		if (Item->_Items.size() != 0)
		{
			auto Result = std::find(Item->_Items.begin(), Item->_Items.end(), This);

			Item->_Current = (Result == Item->_Items.end()) ? 0u : static_cast<UINT>(Result - Item->_Items.begin());
		}
...
LRESULT __stdcall ComboBox::WndProc(HWND Window, UINT Message, WPARAM wp, LPARAM lp)
{
	...
	case WM_KILLFOCUS:
	{
		...
		{
			SendMessageW(Info->MsgTarget, ComboBox_SetNewText, reinterpret_cast<WPARAM>(Info->Return.c_str()), 0);
...
void ComboBox::Items(const std::vector<String>& New)
{
	_Items = New;
}
...
bool ComboBox::Seek(String New)
{
	...
	auto Result = std::find(_Items.begin(), _Items.end(), New);
	if (Result != _Items.end());
		return false;

	_Current = static_cast<int>(Result - _Items.begin());
	Redraw();
...
void ComboBox::UpdateDrawer()
{
	if (IsPopout)
	{
		...
		{
			Info->Objects.push_back(new Button(XCoord, YCoord, Width, Height, _Items[i], Info->Grid, (HMENU)1, ins, Temp, _TextStyle, TA_LeftAlignment));
			YCoord += 5 + Height;
		}
...
LRESULT ComboBox::Paint()
{
	...
	HFONT Font = CreateFontW(-MulDiv(_TextStyle.FontSize, GetDeviceCaps(Dc, LOGPIXELSY), 72), 0, 0, 0, (_TextStyle.Bold ? FW_BOLD : 0), _TextStyle.Italic, false, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_DONTCARE, static_cast<LPCWSTR>(_TextStyle.FontFamily.c_str()));
	...
	const ::Char* Text = _Items[_Current].c_str();
	int TextLen = static_cast<int>(_Items[_Current].size());
