...
CheckableButton::CheckableButton(int X, int Y, int Width, int Height, HWND Parent, HINSTANCE ins, HMENU ID, bool IsChecked, String Text, CheckableButtonType Type, StyleSheet Style, TextStyleSheet TextStyle)
{
	...
	_Base = CreateWindowEx(0l, MAKEINTATOM(_ThisAtom), L"", WS_CHILD | WS_VISIBLE, X, Y, Width, Height, Parent, ID, ins, NULL);
...
{	
	CheckableButton* This = reinterpret_cast<CheckableButton*>(GetWindowLongPtr(Window, GWLP_USERDATA));
	if (!This)
...
void CheckableButton::SetCheckState(bool New)
{
	...
	SendMessage(GetParent(_Base), WM_COMMAND, reinterpret_cast<WPARAM>(GetMenu(_Base)), static_cast<LPARAM>(_Check));
}

LRESULT CheckableButton::KeyDown(WPARAM wp)
{
	if (wp == VK_RETURN)
		return SendMessage(_Base, WM_LBUTTONDOWN, 0, 0);
	else if (wp == VK_TAB)
	{
...
LRESULT CheckableButton::Paint()
{
	...
	HFONT Font = CreateFont(-MulDiv(_TextStyle.FontSize, GetDeviceCaps(Dc, LOGPIXELSY), 72), 0, 0, 0, (_TextStyle.Bold ? FW_BOLD : 0), _TextStyle.Italic, false, false, ANSI_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, ANTIALIASED_QUALITY, FF_DONTCARE, _TextStyle.FontFamily.c_str());
	...
	size_t TextLen = _Text.size();

	SIZE TextSize;
	GetTextExtentPoint32(Dc, Text, TextLen, &TextSize);
