...
Button::Button(int X, int Y, int Width, int Height, String Text, HWND Parent, HMENU ID, HINSTANCE ins, StyleSheet _Style, TextStyleSheet _Text, TextAlignment Alignment)
{
	...
	_Base = CreateWindowEx(0l, L"ButtonClass", L"", WS_CHILD | WS_VISIBLE, X, Y, Width, Height, Parent, ID, ins, NULL);
...
LRESULT Button::Paint()
{
	...
	DrawText(Dc, Text, TextLen, &WndRect, (_Alignment == TA_CenterAlignment ? DT_CENTER : _Alignment == TA_LeftAlignment ? DT_LEFT : DT_RIGHT) | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS | DT_NOPREFIX);
...
{
	_Clicked = true;
	SendMessage(::GetParent(_Base), WM_COMMAND, reinterpret_cast<WPARAM>(GetMenu(_Base)), reinterpret_cast<LPARAM>(this));
