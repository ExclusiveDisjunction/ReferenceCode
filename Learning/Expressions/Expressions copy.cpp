...
void InitOperators()
{
	...
	//Note that the braces have the highest priority. In the functions they are treated different then the other operators. 
	Operators = {
		make_pair(1, '+'),
		make_pair(1, '-'),
		make_pair(2, '*'),
...
{
	//Uses std::find_if to lookup the current operator in the operators list. 
	vector<pair<int, char>>::iterator Result = std::find_if(Operators.begin(), Operators.end(), [oper](const pair<int, char>& obj) -> bool
		{
			return obj.second == oper;
		});

	return Result == Operators.end() ? 0 : Result->first;
}
int CompareOperPrecidence(char oper1, char oper2)
{
	int Result1 = GetOperPrecidence(oper1), Result2 = GetOperPrecidence(oper2); //Retrives the precidiences

	if (Result1 == 0 || Result2 == 0) //Base case, no comparison
		return -2;
...
{
	if (!IsOperator(oper)) //Not valid
		throw FormatError(string(oper, 1), "The operator provided was not an operator.");
