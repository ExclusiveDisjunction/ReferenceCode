...
{	
	...
	LRESULT Startup::Paint()
	{
		...
		Image* img;
		{
			...
			HRSRC hResource = FindResourceW(ins, MAKEINTRESOURCE(Mason_Splash), L"PNG");
		...
		SolidBrush HeaderBrush(Gdiplus::Color(0xFF, Accent1.R(), Accent1.G(), Accent1.B()));
		...
		Graph->DrawString(L"Mason", -1, &TitleFont, TitleTextOrig, &WhiteBrush);
		...
		String FooterText = L"Mason | Artwork By: Hollan C. Sellars";
		Font FooterFont(&StdFam, 13);

		RectF FooterRect(0.0f, Area.Height - 30.0f, static_cast<float>(Area.Width), 30.0f);
		SolidBrush FooterBrush(Gdiplus::Color(0xc5292929));
