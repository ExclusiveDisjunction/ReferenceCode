...
namespace Mason
{
	...
	namespace UI
	{
		...
		public:
			...
			ComponentViewer* operator[](unsigned int Index) const;
			ComponentViewer* Item(unsigned it Index) const { return operator[](Index); }
			unsigned int const& Size = _Size;

			void Clear();

			Core::Vector<Files::Component*> RetriveFromList() const; //Returns only those that are selected.
			Core::Vector<Files::Component*> RetriveFromList(Core::Vector<ComponentViewer*>& Selected) const; //Returns only those that are selected.
			Core::Vector<Files::Component*> GetAllComponents() const;
