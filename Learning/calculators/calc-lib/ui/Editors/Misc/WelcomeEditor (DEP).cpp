...
//#include "..\..\..\Configuration\ConfigInstance.h"
#include "..\EditorRegistry.h"
//#include "..\..\..\IO\FileControl.h"
//#include "..\..\..\IO\SimipleFile.h"
...
namespace Mason::UI::Editors::Misc
{
	...
	void WelcomeEditor::RefreshList()
	{
		...
		StringList Paths = StringList();//RecentInstance->GetRecentsForUI();
		this->Paths = StringList(); //RecentInstance->RecentPaths;
	...
	LRESULT WelcomeEditor::Command(WPARAM wp, LPARAM lp)
	{
		...
		{
		case 4: //Open
			if (true) //SessionControl::Open(ins))
				EditorRegistry::CloseEditor(this);
			break;
		case 5: //New
			if (true) //SessionControl::NewFile(ins))
				EditorRegistry::CloseEditor(this);
			break;
		case 6: //Load Last
		{
			String Path = String(); // InsInstance->LastLoaded;
			if (true) //SessionControl::Open(Path, ins))
				DestroyWindow(_Base);
			else
