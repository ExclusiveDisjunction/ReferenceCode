...
namespace Mason::UI::Controls
{
	...
	EditorButton::EditorButton(int XCoord, int YCoord, int Width, int Height, HWND Parent, HINSTANCE ins, Editor* Source, EditorHost* Host, Color BaseBk)
	{
		if (!_ThisAtom)
			InitBase(ins);
	...
	LRESULT EditorButton::Paint()
	{
		...
		Color BorderColor = Source->EditorState & EDS_AppendError ? 0xFF'FF'00'00 : 0xFF'FF'FF'FF;
		Color BkColor = (HasMouse || _Clicked || IsWindowVisible(*Source)) ? Accent3 : _BaseBk;
		Color ForegroundColor = 0xFF'FF'FF'FF;
