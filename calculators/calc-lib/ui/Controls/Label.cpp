...
namespace Mason::UI::Controls
{
	...
	Label::Label(int X, int Y, int Width, int Height, HWND Window, HINSTANCE ins, String Text, Color BaseBk, TextStyleSheet Style, bool ShowScroll)
	{
		if (!_ThisAtom)
			InitBase(ins);
	...
	LRESULT Label::Paint()
	{
		...
		Color Foreground = _Style.Foreground;
		if (!IsEnabled)
