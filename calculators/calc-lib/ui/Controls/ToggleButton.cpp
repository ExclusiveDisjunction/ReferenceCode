...
namespace Mason::UI::Controls
{
	...
	LRESULT ToggleButton::Paint()
	{
		...
		Color HoverColor = _Style.BorderBrush;
		HoverColor.Reduce(0.7f);

		Color BkColor = (_Checked) ? _Style.BorderBrush : (_HasMouse ? HoverColor : _Style.Background), ForegroundColor = _TextStyle.Foreground;
		if (!IsEnabled)
		{
			BkColor.Reduce(0.7f);
