...
namespace Mason::UI::Controls
{
	...
	void TextBox::AdjustCaretPos()
	{
		...
		if (_HasScrolls)
		{
			unsigned int PrePos = CaretPos.x;

			if (CaretPos.y != 0)
			{
				StringList Lines = String(Text).Split(L'\n');
				for (unsigned int i = 0; i < Lines.Size && i < static_cast<unsigned int>(CaretPos.y - 1); i++)
	...
	{
		auto LongestLine = [](StringList& Lines) -> unsigned int
		{
			unsigned int Last = 0u;
			for (unsigned int i = 0; i < Lines.Size; i++)
			{
				unsigned int Current = Lines[i].Length();
				if (Current > Last)
					Last = Current;
			}
		...
		StringList Lines = String(Text).Split(L'\n');
		unsigned int LargestLen = LongestLine(Lines);
	...
	LRESULT TextBox::KeyDown(WPARAM wp)
	{
		if (GetKeyState(VK_CONTROL) & 0x8000)
		{
			...
			case L'V':
			{
				...
				int Pos = CaretPos.x;
				StringList Lines = String(Text).Split(L'\n');
				for (unsigned int i = 0; i < Lines.Size && i < static_cast<unsigned int>(CaretPos.y); i++)
					Pos += lstrlenW(static_cast<LPCWSTR>(Lines[i])) + 1;

				int Selection = SelectionPos.x;
				for (unsigned int i = 0; i < Lines.Size && i < static_cast<unsigned int>(SelectionPos.y); i++)
	...
	LRESULT TextBox::Paint()
	{
		...
		Color BkColor = _Style.Background, BorderColor = _Style.BorderBrush, ForegroundColor = _TextStyle.Foreground;
		...
		auto Contains = [](wchar_t* Items, wchar_t Search) -> bool
		{
			...
			for (unsigned int i = 0; i <= static_cast<unsigned int>(StrLen); i++)
			{
				if (Items[i] == Search)
					return true;
			}
	...
	void TextBox::AppendKey(wchar_t New)
	{
		...
		{
			unsigned int TempSize = _Compacity - Pos;
		...
		else
		{
			...
			unsigned int TempSize = _Compacity - Two;
			wchar_t* TempText = static_cast<wchar_t*>(malloc(sizeof(wchar_t) * TempSize));
			memset(TempText, 0, sizeof(wchar_t) * TempSize);
